...
def main():
    ...
    sumAccuracy=0
    for i in range(4):
        indices = random.sample(range(len(trainingData['data'])),100)
        newTrainingInstances = []
        for i in indices:
    ...
    #     print 'Sr.\t Actual Class\t Predicted Class'
        for instance in testData['data']:
            predicted = predictClass(instance, attributes, len(newTrainingInstances))
        ...
        sumAccuracy+=(correctCount/float(i))*100
